...
def check_root_package(path: Path) -> None:
    ...
    assert_equal(scripts.get("build"), "npm run build:web", "root package build script")
...
def check_deployment_docs(path: Path) -> None:
    ...
    for needle, label in [
        ...
        ("Build output directory: apps/web/dist", "repository-root Pages output directory"),
        ("VITE_API_BASE_URL", "Pages API base environment variable"),
        ("The repository root `build` script delegates to `npm run build:web`", "root build compatibility note"),
    ]:
